...
def graph(glist):
    pairs = [(g1, g2) for g1, g2 in product(glist, glist) if g1 != g2 and g1.isCyclic(g2)]
    groups = groupby([map(glist.index, e) for e in pairs], first)
    return {k: map(last, v) for k, v in groups}

def solution(blist):
    glist = map(Guard, blist)
    matches = matching(graph(glist))
    return len(glist) - len(matches)
